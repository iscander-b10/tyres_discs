...
| Race guards | `searchRequestIdRef`, `foregroundRequestIdRef`, `loadRequestIdRef`, `workspaceKeyRef`, `mountedRef` | stale search/facets/workspace/unmount; spinner «Найти» |
...
`CatalogShowcase`; уже он отдельно вызывает `getCatalogShowcase`.
