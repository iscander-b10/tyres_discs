...
Откройте `http://127.0.0.1:5000/` (порт: `PORT` или аргумент скрипта, по умолчанию 5000). Скрипт сам открывает браузер; терминал нужно **оставить открытым**. На Windows лучше `127.0.0.1`, не `localhost` (`localhost` часто резолвится в `::1`). Basename берётся из pathname `homepage` в `package.json` (для `https://silvertyres.pro` — корень).
