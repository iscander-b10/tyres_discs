...
Обе empty-ветки списка: `min-height: 420px`, контент в верхней трети — тот же ритм, что у `CatalogSearchEmptyHint`. Title-filter empty чистит локальный поиск по названию (`handleSearchClear`), не форму фильтров. Сетка и title-filter empty меняются через `CatalogResultsFade` (opacity 50ms на весь блок, без stagger карточек). Toolbar «Поиск по названию» не анимируется.
