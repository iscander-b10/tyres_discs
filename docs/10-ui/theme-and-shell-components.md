...
Компонент рендерит нативную кнопку с `role="switch"`, `aria-checked` и label
следующего действия. После клика он:

1. вычисляет противоположное appearance;
2. ставит локальный `isPending`;
3. вызывает callback;
4. на 500 ms блокирует повторный клик, пока завершается CSS-сцена
   переключателя;
5. очищает timeout при unmount.

Эти 500 ms относятся только к pending-lock визуального switch. Сама тема
документа применяется сразу.
