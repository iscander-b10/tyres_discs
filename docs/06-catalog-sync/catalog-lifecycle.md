...
Локальная база пуста, поэтому совпадение версии в старом `localStorage` не блокирует bootstrap. Клиент скачивает snapshot, создаёт stores, применяет команды и фиксирует `snapshotVersion`, затем прогревает RAM обеих категорий. Пока снимок не в IndexedDB, AppShell держит полноэкранную шторку: пользователь не видит рабочий сайт. После `phase: 'ready'` шторка ещё не снимается, пока активная витрина не покажет полки; skeleton в момент снятия шторки пользователю не показывают. Empty «Каталог ещё загружается» в витрине нет — при пустом store витрина по-прежнему рисует skeleton, но только под шторкой или на warm start.

### Повторный запуск

Клиент читает meta и persisted-версию из `metadata`. Если `meta.version <= localVersion` и каталог не пуст, возвращается `up-to-date`; snapshot и товары не перезаписываются. Шторки нет: autosync идёт тихо.
...
До commit локальный каталог не изменяется. На warm start host не удаляет старые товары и повторит проверку при следующем триггере, в том числе событии `online`. На cold start ошибка остаётся в шторке с кнопкой «Повторить».
