...
Локальная база пуста, поэтому совпадение версии в старом `localStorage` не блокирует bootstrap. Клиент скачивает snapshot, создаёт stores, применяет команды и фиксирует `snapshotVersion`, затем прогревает RAM обеих категорий. Пока снимок не в IndexedDB, AppShell держит полноэкранную шторку: пользователь не видит рабочий сайт. Empty «Каталог ещё загружается» в витрине нет — при пустом store витрина показывает skeleton.
