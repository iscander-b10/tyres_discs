...
Гость видит landing на `/`: единый каталог под магазин (поиск, доукомплектация, WhiteLabel, устройства, цены, два режима), кнопку входа и **«Посмотреть демо»** (`/demo`). Секции лендинга прокручиваются как слайды: колесо, жест и стрелки останавливают экран на следующей секции. При `prefers-reduced-motion` остаётся обычный скролл. Финальный блок ведёт на демо и контакты разработчика (`SITE_DEVELOPER_TELEGRAM`, `SITE_DEVELOPER_PHONE`). Телефон магазина остаётся в шапке и подвале. Защищённый staff URL перенаправляет гостя на query-modal `/?login=1`. На `/demo*` «Войти»/«Выйти» скрыты.
