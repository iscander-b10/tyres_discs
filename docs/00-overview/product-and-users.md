...
Гость видит landing на `/`, телефон магазина, описание возможностей и кнопку
входа. Защищённый URL перенаправляет его на query-modal `/?login=1`. Корзина и
manager/client switch гостю недоступны; при этом AppShell принудительно
публикует для гостевого presentation layer `clientMode = true`.
...
| --- | --- |
| «Посмотреть демо» | Disabled UI stub; `isDemo = false` |
