...
`CatalogBootstrapOverlay` рендерится из `AppShellProvider` порталом на `document.body`. Это не Ant Design Modal: клик по маске и Escape не закрывают шторку. z-index (`--z-catalog-bootstrap`: 1300) выше хедера и ModeToggle. Фон `--color-overlay-gate` темнее `--color-overlay-strong`. Крупный процент (или МБ) — статичный `catalogBootstrapHeadline` в центре (64px, `--color-accent`, не `--color-cta`); вокруг него indeterminate-спиннер (дуга бежит по часовой, заполнение 0–100 не рисуется). Линейного бара нет. Ожидание lock не кладёт длинный текст внутрь круга: спиннер крутится, 0% не показывают, подпись снизу. Ошибка cold start показывается в шторке без спиннера и без бара, кнопка «Повторить» вызывает `retryCatalogBootstrap`. Focus trap, `inert` на `#root` и `overflow: hidden` на `body` держатся, пока шторка смонтирована, включая 50ms exit.
