...
- Маршрут `GET /v2/metrics/load` — опциональные метрики загрузки каталога (логи админа; UI-кнопки больше нет).
...
График **Requests** в мониторинге считает все вызовы функции, включая картинки карточек. Это не «сколько раз синхронизировали каталог».

Нужные строки — в **Cloud Function → Логи**, по одной JSON-строке:

| `event` | Смысл |
| --- | --- |
| `load-start` | Старт загрузки каталога (если кто-то дернул metrics) |
...
Маршруты каталога (`/v2/catalog/{storeId}/meta|snapshot`) описаны в том же `apigw.yaml` — см. [`../catalog-sync/README.md`](../catalog-sync/README.md).
